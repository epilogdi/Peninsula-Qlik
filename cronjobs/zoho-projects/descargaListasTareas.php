...
function storeTasklists($project){ 
  global $mongoClient;
  global $database;
  ...
  $url = "https://projectsapi.zoho.com/restapi/portal/$ENV_ZOHO_PROJECTS_PORTAL_ID/projects/$project->id/tasklists/?index=$page&range=200";
  ...
  try {
    ...
    if($response){
      ...
      $mongoClient->$database->tasklists->insertMany($records);
      $page++;
      $page=$page*100;
      storeTasklists($project);
    }
...
$mongoClient->$database->tasklists->drop();
...
$cron = new stdClass(); 
$cron->type="Descarga Listas de Tareas";
